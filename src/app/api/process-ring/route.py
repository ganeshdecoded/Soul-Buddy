...
@app.post("/api/process-ring")
async def process_ring(hand_file: UploadFile = File(...), ring_file: UploadFile = File(...)):
    # Read hand image
    hand_contents = await hand_file.read()
    hand_array = np.frombuffer(hand_contents, np.uint8)
    hand_image = cv2.imdecode(hand_array, cv2.IMREAD_COLOR)

    # Read ring image
    ring_contents = await ring_file.read()
    ring_array = np.frombuffer(ring_contents, np.uint8)
    ring_image = cv2.imdecode(ring_array, cv2.IMREAD_UNCHANGED)
